...
import 'package:blueprint/src/presentation/screen/components/authorities_lov_widget.dart';
import 'package:blueprint/src/presentation/screen/components/editor_form_mode.dart';
import 'package:blueprint/src/presentation/screen/components/responsive_form_widget.dart';
import 'package:blueprint/src/presentation/screen/components/submit_button_widget.dart';
import 'package:blueprint/src/presentation/screen/components/user_form_fields.dart';
...
class UserEditorScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final initialEvent =
        userId.isNotEmpty ? UserFetchEvent(userId) : const UserEditorInit();
    bloc.add(initialEvent);
    return UserEditorWidget(mode: mode);
  }
}

_showMessage(BuildContext context, GlobalKey<ScaffoldState> scaffoldKey,
...
class UserEditorWidget extends StatelessWidget {
    ...
    return BlocConsumer<UserBloc, UserState>(
      ...
      listener: (context, state) {
        if (state.status == UserStatus.loading) {
          _showMessage(context, _scaffoldKey, S.of(context).loading,
              S.of(context).loading);
        }

        if (state.status == UserStatus.success) {
          _showMessage(context, _scaffoldKey, S.of(context).success,
              S.of(context).success);
        }

        if (state.status == UserStatus.failure) {
          _showMessage(context, _scaffoldKey, S.of(context).failed,
              S.of(context).failed);
        }
      },
  ...
  Future<void> _handlePopScope(bool didPop, Object? data,
      [BuildContext? contextParam]) async {
  ...
  List<Widget> _buildFormFields(BuildContext context, UserState state) {
    ...
    return [
      UserFormFields.usernameField(context, state.data?.login,
