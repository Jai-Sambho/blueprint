...
class UserBloc extends Bloc<UserEvent, UserState> {
  ...
  FutureOr<void> _onEditorInit(
      UserEditorInit event, Emitter<UserState> emit) async {
  ...
  FutureOr<void> _onSubmit(
      UserSubmitEvent event, Emitter<UserState> emit) async {
    _log.debug(
        "BEGIN: onSubmit UserSubmitEvent event: {}", [event.user.toString()]);
    ...
    try {
      ...
      _log.debug(
          "END:onSubmit UserSubmitEvent event success: {}", [user.toString()]);
    } catch (e) {
      emit(state.copyWith(status: UserStatus.failure));
      _log.error(
          "END:onSubmit UserSubmitEvent event error: {}", [e.toString()]);
    }
  }
  ...
  FutureOr<void> _onDelete(
    ...
    try {
      if (event.id == "user-1") {
        emit(state.copyWith(
            status: UserStatus.failure, err: "Admin user cannot be deleted"));
  ...
  FutureOr<void> _onFetchUser(
    ...
    try {
      ...
      emit(state.copyWith(status: UserStatus.failure, err: e.toString()));
      _log.error(
          "END:onFetchUser FetchUserEvent event error: {}", [e.toString()]);
    }
  }
  ...
  FutureOr<void> _onSearch(
    ...
    try {
      if ((event.name == null || event.name == "") &&
        ...
        emit(state.copyWith(
            status: UserStatus.searchSuccess, userList: entities));
        _log.debug(
        ...
        final entities = await _repository.listByNameAndRole(
            event.page, event.size, event.name!, event.authorities!);
        emit(state.copyWith(
            status: UserStatus.searchSuccess, userList: entities));
        _log.debug(
        ...
        final entities = await _repository.listByAuthority(
            event.page, event.size, event.authorities!);
        emit(state.copyWith(
            status: UserStatus.searchSuccess, userList: entities));
        _log.debug(
  ...
  FutureOr<void> _onSaveComplete(
      UserSaveCompleteEvent event, Emitter<UserState> emit) async {
  ...
  FutureOr<void> _onViewComplete(
      UserViewCompleteEvent event, Emitter<UserState> emit) async {
