...
void main() {
  ...
  group("RegisterScreen Test", () {
    ...
    testWidgets("Validate Form Fields Enter Text", (tester) async {
      ...
      await tester.enterText(
          find.byType(FormBuilderTextField).last, "test@test.com");
    });
    ...
    testWidgets("Validate Form Fields Enter Text and submit", (tester) async {
      ...
      await tester.enterText(
          find.byKey(registerEmailTextFieldKey), "test@test.com");
  ...
  testWidgets('Register form validation and submission test',
    ...
    when(mockRegisterBloc.add(const RegisterFormSubmitted(data: user)))
        .thenReturn(null);
  ...
  testWidgets('Register loaded state test', (WidgetTester tester) async {
    ...
    when(mockRegisterBloc.state).thenReturn(const RegisterCompletedState(
        user:
            User(firstName: 'test', lastName: 'test', email: 'test@test.com')));
    //await tester.pump();
    ...
    verify(mockRegisterBloc.add(const RegisterFormSubmitted(
            data: User(
                firstName: 'test', lastName: 'test', email: 'test@test.com'))))
        .called(1);
  });
  ...
  testWidgets('Register error state test', (WidgetTester tester) async {
    ...
    when(mockRegisterBloc.stream).thenAnswer((_) =>
        Stream.fromIterable([const RegisterErrorState(message: 'Error')]));
    ...
    expect(find.byType(RegisterScreen), findsOneWidget);
    verifyNever(mockRegisterBloc.add(const RegisterFormSubmitted(
        data: User(
            firstName: 'test', lastName: 'test', email: 'test@test.com'))));
  });
}
