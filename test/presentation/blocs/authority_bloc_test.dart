...
void main() {
  ...
  group("AuthorityState", () {
    ...
    test("supports value comparisons", () {
      expect(const AuthorityState(authorities: authorities, status: status),
    ...
    test("AuthorityLoadSuccessState", () {
      expect(const AuthorityLoadSuccessState(authorities: authorities),
          const AuthorityLoadSuccessState(authorities: authorities));
    });

    test("AuthorityLoadFailureState", () {
      expect(const AuthorityLoadFailureState(message: "test"),
    ...
    test("AuthorityState copyWith", () {
      ...
      expect(const AuthorityState().copyWith(authorities: authorities),
          const AuthorityState(authorities: authorities));
      expect(const AuthorityState().copyWith(status: AuthorityStatus.success),
    ...
    test("AuthorityState props", () {
      ...
      expect(const AuthorityState(authorities: authorities).props,
          [AuthorityStatus.initial, authorities]);

      expect(
          const AuthorityInitialState().props, [AuthorityStatus.initial, []]);
      expect(
          const AuthorityLoadingState().props, [AuthorityStatus.loading, []]);
      expect(const AuthorityLoadSuccessState(authorities: authorities).props,
          [AuthorityStatus.success, authorities]);
      expect(const AuthorityLoadFailureState(message: "test").props,
          [AuthorityStatus.failure, "test"]);
    });
  });
  ...
  group("AuthorityBloc", () {
    ...
    group("AuthorityLoad", () {
      ...
      Future<List<String?>> output =
          Future<List<String?>>.value(authoritiesMap);
      const event = AuthorityLoad();
      const loadingState = AuthorityLoadingState();
      final successState =
          AuthorityLoadSuccessState(authorities: authoritiesMap);
      const failureState =
          AuthorityLoadFailureState(message: "Exception: Error");
