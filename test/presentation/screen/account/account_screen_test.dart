...
import 'package:blueprint/src/presentation/screen/components/submit_button_widget.dart';
import 'package:blueprint/src/presentation/screen/user/bloc/user.dart';
...
void main() {
  ...
  Widget buildTestableWidget() {
    final router =
        GoRouter(routes: AccountRoutes.routes, initialLocation: '/account');
  ...
  group('AccountScreen Basic UI Tests', () {
    testWidgets('Should render AppBar correctly', (tester) async {
      ...
      expect(find.byKey(const Key('accountScreenAppBarBackButtonKey')),
          findsOneWidget);
    });
    ...
    testWidgets('Should render form fields correctly', (tester) async {
      ...
      expect(
          find.byKey(const Key('userEditorFirstNameFieldKey')), findsOneWidget);
      expect(
          find.byKey(const Key('userEditorLastNameFieldKey')), findsOneWidget);
      expect(find.byKey(const Key('userEditorEmailFieldKey')), findsOneWidget);
      expect(
          find.byKey(const Key('userEditorActivatedFieldKey')), findsNothing);
    });
  });
  ...
  group('AccountScreen State Tests', () {
    testWidgets('Should display loading indicator when in loading state',
      ...
      accountStateController
          .add(const AccountState(status: AccountStatus.loading));
  ...
  group('AccountScreen Form Operations', () {
    testWidgets(
      ...
      await tester.enterText(
          ...
          'Updated First');
      await tester.enterText(
          find.byKey(const Key('userEditorLastNameFieldKey')), 'Updated Last');
    ...
    testWidgets(
      ...
      expect(
          find.descendant(
              of: find.byType(SnackBar), matching: find.text(S.current.failed)),
          findsOneWidget,
  ...
  group('AccountScreen Navigation Tests', () {
    testWidgets(
      ...
      await tester.enterText(
          find.byKey(const Key('userEditorFirstNameFieldKey')), 'Changed Name');
  ...
  group('AccountScreen State Management Tests', () {
    testWidgets('Should show snackbar messages for different states',
      ...
      // Test success state
      accountStateController
          .add(const AccountState(status: AccountStatus.success));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 50));
      ...
      // Test failure state
      accountStateController
          .add(const AccountState(status: AccountStatus.failure));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 50));
    ...
    testWidgets('Should show snackbar messages for different states2',
      ...
      await tester
          .pump(const Duration(milliseconds: 750)); // Animation midpoint
