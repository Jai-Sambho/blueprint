...
class UserRepository {
  ...
  Future<User?> retrieve(String id) async {
    ...
    final pathParams = id;
    final httpResponse =
        await HttpUtils.getRequest("/admin/$_resource", pathParams: pathParams);
    final response = User.fromJsonString(httpResponse.body)!;
    _log.debug(
        "END:getUser successful - response.body: {}", [response.toString()]);
    return response;
  }
  ...
  Future<User?> create(User user) async {
    ...
    final httpResponse =
        await HttpUtils.postRequest<User>("/admin/$_resource", user);
    final response = User.fromJsonString(httpResponse.body);
  ...
  Future<User?> update(User user) async {
    ...
    final httpResponse =
        await HttpUtils.putRequest<User>("/admin/$_resource", user);
    final response = User.fromJsonString(httpResponse.body);
  ...
  Future<List<User>> list(
      {int page = 0,
      int size = 10,
      List<String> sort = const ["id,desc"]}) async {
    _log.debug("BEGIN:getUsers repository start - page: {}, size: {}, sort: {}",
        [page, size, sort]);
    final queryParams = {
      "page": page.toString(),
      "size": size.toString(),
    ...
    final httpResponse = await HttpUtils.getRequest("/admin/$_resource",
        queryParams: queryParams);
    final response = User.fromJsonStringList(httpResponse.body);
    _log.debug(
        "END:getUsers successful - response list size: {}", [response.length]);
    return response;
  }

  /// Find user method that findUserByAuthorities a user
  Future<List<User>> listByAuthority(
      int page, int size, String authority) async {
    _log.debug(
    ...
    final response = await HttpUtils.getRequest("/admin/$_resource/authorities",
  ...
  Future<List<User>> listByNameAndRole(
      int page, int size, String name, String authority) async {
    _log.debug(
    ...
    final response = await HttpUtils.getRequest("/admin/$_resource/filter",
        queryParams: queryParams);
    var result = JsonMapper.deserialize<List<User>>(response.body)!;
  ...
  Future<void> delete(String id) async {
    ...
    final pathParams = id;
    final httpResponse = await HttpUtils.deleteRequest("/admin/$_resource",
