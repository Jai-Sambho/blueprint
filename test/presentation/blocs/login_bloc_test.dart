...
import 'package:blueprint/src/data/models/jwt_token.dart';
import 'package:blueprint/src/data/models/send_otp_request.dart';
import 'package:blueprint/src/data/models/user_jwt.dart';
import 'package:blueprint/src/data/models/verify_otp_request.dart';
import 'package:blueprint/src/data/repository/account_repository.dart';
import 'package:blueprint/src/data/repository/login_repository.dart';
import 'package:blueprint/src/presentation/screen/login/bloc/login.dart';
...
void main() {
  ...
  group("LoginState", () {
    ...
    test("supports value comparisons", () {
      ...
      const state =
          LoginState(status: LoginStatus.initial, passwordVisible: false);
    ...
    test("LoginErrorState", () {
      expect(const LoginErrorState(message: "test"),
    ...
    test("copyWith replaces non-null parameters", () {
      const state =
          LoginState(status: LoginStatus.initial, passwordVisible: false);
      const secondState =
          LoginState(status: LoginStatus.success, passwordVisible: true);
      expect(state.copyWith(status: LoginStatus.success, passwordVisible: true),
          secondState);
    });

    test("Initial state is LoginState", () {
      expect(LoginBloc(repository: MockLoginRepository()).state,
          const LoginState());
    });
  ...
  group("LoginEvent", () {
    test("supports value comparisons", () {
      expect(const LoginFormSubmitted(username: "test", password: "test"),
    ...
    test("props", () {
      expect(const LoginFormSubmitted(username: "test", password: "test").props,
          ["test", "test"]);
  ...
  group("LoginBloc", () {
    ...
    group("LoginFormSubmitted", () {
      ...
      final successState = LoginLoadedState(
          username: input.username!, password: input.password!);
      const failureState =
          LoginErrorState(message: "Login API Error: Operation Unauthorized");
  ...
  group('LoginBloc test 2', () {
    ...
    group('LoginFormSubmitted', () {
      ...
      blocTest<LoginBloc, LoginState>(
        ...
        setUp: () => when(repository.authenticate(input))
    ...
    group('SendOtpRequested', () {
      ...
      blocTest<LoginBloc, LoginState>(
        ...
        expect: () => [
          isA<LoginLoadingState>()
              .having((state) => state.username, 'username', email),
          isA<LoginOtpSentState>()
      ...
      blocTest<LoginBloc, LoginState>(
        ...
        setUp: () => when(repository.sendOtp(request))
    ...
    group('VerifyOtpSubmitted', () {
      ...
      blocTest<LoginBloc, LoginState>(
        ...
        setUp: () => when(repository.verifyOtp(request))
            .thenAnswer((_) async => const JWTToken(idToken: null)),
        build: () => LoginBloc(repository: repository),
        act: (bloc) => bloc.add(event),
        expect: () => [
