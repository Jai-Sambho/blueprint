...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  FutureOr<void> _onSubmit(
    ...
    _log.debug(
        "BEGIN: onSubmit LoginFormSubmitted event: {}", [event.username]);
    ...
    try {
      ...
      _log.error(
          "END:onSubmit LoginFormSubmitted event error: {}", [e.toString()]);
    }
  }
  ...
  Future<void> _onSendOtpRequested(
    ...
    _log.debug(
        "BEGIN: onSendOtpRequested SendOtpRequested event: {}", [event.email]);
  ...
  Future<void> _onVerifyOtpSubmitted(
      VerifyOtpSubmitted event, Emitter<LoginState> emit) async {
    _log.debug("BEGIN: onVerifyOtpSubmitted VerifyOtpSubmitted event: {}",
        [event.email]);
    emit(state.copyWith(status: LoginStatus.loading));
    try {
      final token = await _repository
          .verifyOtp(VerifyOtpRequest(email: event.email, otp: event.otpCode));
