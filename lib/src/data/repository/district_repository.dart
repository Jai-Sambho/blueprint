...
class DistrictRepository {
  ...
  Future<List<District?>> listByCity(String cityId) async {
    _log.debug(
        "BEGIN:getDistrictsByCity repository start - cityId: {}", [cityId]);
    if (cityId.isEmpty) {
      throw BadRequestException("City id null");
    }
    final pathParams = cityId;
    final httpResponse = await HttpUtils.getRequest("/$_resource/cities",
  ...
  Future<District?> create(District district) async {
    _log.debug(
        "BEGIN:createDistrict repository start : {}", [district.toString()]);
