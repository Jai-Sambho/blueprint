...
import 'package:blueprint/src/configuration/padding_spacing.dart';
import 'package:blueprint/src/presentation/screen/components/confirmation_dialog_widget.dart';
...
class ChangePasswordScreen extends StatelessWidget {
  ...
  void _onSubmit(BuildContext context, ChangePasswordState state) {
    ...
    if (!(_formKey.currentState?.validate() ?? false)) {
      debugPrint("validate");
      _showSnackBar(
          context, S.of(context).failed, const Duration(milliseconds: 1000));
      return;
    }

    if (!(_formKey.currentState?.isDirty ?? false)) {
      debugPrint("no changes made");
      _showSnackBar(context, S.of(context).no_changes_made,
  ...
  Future<void> _handlePopScope(bool didPop, Object? data,
    ...
    final shouldPop = await ConfirmationDialog.show(
            context: context, type: DialogType.unsavedChanges) ??
        false;
