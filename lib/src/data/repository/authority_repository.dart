...
class AuthorityRepository {
  ...
  Future<Authority?> create(Authority authority) async {
    _log.debug(
        "BEGIN:createAuthority repository start : {}", [authority.toString()]);
    if (authority.name == null || authority.name!.isEmpty) {
      throw BadRequestException("Authority name null");
    }
    final httpResponse =
        await HttpUtils.postRequest<Authority>("/$_resource", authority);
  ...
  Future<List<String?>> list() async {
    ...
    final httpResponse =
        await HttpUtils.getRequest("/$_resource", queryParams: queryParams);
  ...
  Future<Authority?> retrieve(String id) async {
    ...
    final pathParams = id;
    final httpResponse =
        await HttpUtils.getRequest("/$_resource", pathParams: pathParams);
    final response = Authority.fromJsonString(httpResponse.body);
  ...
  Future<void> delete(String id) async {
    ...
    final pathParams = id;
    final httpResponse =
        await HttpUtils.deleteRequest("/$_resource", pathParams: pathParams);
