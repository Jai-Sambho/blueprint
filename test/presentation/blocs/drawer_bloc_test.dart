...
import 'package:blueprint/src/data/models/menu.dart';
import 'package:blueprint/src/data/repository/login_repository.dart';
import 'package:blueprint/src/data/repository/menu_repository.dart';
import 'package:blueprint/src/presentation/common_widgets/drawer/drawer_bloc/drawer.dart';
import 'package:blueprint/src/utils/menu_list_cache.dart';
...
void main() {
  ...
  group("DrawerState", () {
    ...
    test("supports value comparisons", () {
      expect(const DrawerState(menus: menus, isLogout: isLogout),
    ...
    test("DrawerState copyWith", () {
      ...
      expect(const DrawerState().copyWith(menus: menus),
  ...
  group("DrawerEvent", () {
    test("supports value comparisons", () {
      expect(const LoadMenus(language: "en", theme: AdaptiveThemeMode.light),
  ...
  group("Drawer Bloc", () {
    group("LoadMenu", () {
      ...
      const loadingState =
          DrawerState(menus: [], status: DrawerStateStatus.loading);
      const successState =
          DrawerState(menus: input, status: DrawerStateStatus.success);
      const failureState =
          DrawerState(menus: [], status: DrawerStateStatus.error);
      blocTest<DrawerBloc, DrawerState>(
        ...
        build: () => DrawerBloc(
            loginRepository: loginRepository, menuRepository: menuRepository),
    ...
    group("Logout", () {
      ...
      const successState =
          DrawerState(status: DrawerStateStatus.success, isLogout: true);
