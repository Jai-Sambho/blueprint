...
class LoginRepository {
  ...
  Future<JWTToken?> authenticate(UserJWT userJWT) async {
    _log.debug(
        "BEGIN:authenticate repository start username: {}", [userJWT.username]);
    JWTToken? result;
    ...
    _log.debug(
        "END:authenticate successful - response.body: {}", [result.toString()]);
    return result;
  }
  ...
  Future<JWTToken?> verifyOtp(VerifyOtpRequest request) async {
    ...
    final response = await HttpUtils.postRequest<VerifyOtpRequest>(
        "/authenticate/verify-otp", request,
        headers: headers);
