...
void main() {
  ...
  setUp(() async {
    ...
    when(mockAuthorityBloc.state).thenReturn(const AuthorityLoadSuccessState(
        authorities: ['ROLE_ADMIN', 'ROLE_USER']));
  });
  ...
  group('UserEditorScreen Tests', () {
    testWidgets('Create Mode - Should render empty form', (tester) async {
      ...
      expect(
          find.byKey(const Key('userEditorActivatedFieldKey')), findsOneWidget);
    ...
    testWidgets('Edit Mode - Should load and display user data',
      ...
      when(mockUserRepository.retrieve(userId))
