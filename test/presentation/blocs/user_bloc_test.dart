...
import 'package:blueprint/src/presentation/screen/user/bloc/user_bloc.dart';
...
void main() {
  ...
  group('UserBloc Event Tests', () {
    ...
    blocTest<UserBloc, UserState>(
      ...
      expect: () => [
        isA<UserState>()
            .having((state) => state.status, 'status', UserStatus.loading),
        isA<UserState>()
    ...
    blocTest<UserBloc, UserState>(
      ...
      setUp: () {
        when(repository.create(newUser))
    ...
    blocTest<UserBloc, UserState>(
      ...
      setUp: () {
        when(repository.update(testUser))
    ...
    group('UserSearchEvent Tests', () {
      ...
      blocTest<UserBloc, UserState>(
        ...
        setUp: () {
          when(repository.listByAuthority(0, 10, "ROLE_USER"))
      ...
      blocTest<UserBloc, UserState>(
        ...
        setUp: () {
          when(repository.listByNameAndRole(0, 10, "Test", "ROLE_USER"))
              .thenAnswer((_) async => users);
        },
        build: () => bloc,
      ...
      blocTest<UserBloc, UserState>(
        ...
        expect: () => [
          const UserState(status: UserStatus.loading),
          const UserState(
              status: UserStatus.failure, err: "Exception: Search failed"),
        ],
      );
    });
    ...
    group('UserFetchEvent Tests', () {
      ...
      blocTest<UserBloc, UserState>(
        ...
        expect: () => [
          const UserState(status: UserStatus.loading),
          const UserState(
              status: UserStatus.failure, err: "Exception: Fetch failed"),
        ],
      );
    });
    ...
    group('UserDeleteEvent Tests', () {
      ...
      blocTest<UserBloc, UserState>(
        ...
        expect: () => [
          const UserState(status: UserStatus.loading),
          const UserState(
              status: UserStatus.failure, err: "Exception: Delete failed"),
        ],
      );
    });
